...
def mlp(data,labels,valid_data,valid_labels,h1 = 10,l_rate = 0.01,init_var=0.1,
    ...
    # #pl.subplot(2,1,1)
    # pl.title("Errors with respect to the epoch")
    # pl.plot(arange(len(error[0])),error[0])
    # #pl.title("training logistic error")
    # #pl.subplot(2,1,2)
    pl.plot(arange(len(error[1])),error[1])
    # #pl.title("validation logistic error")
