...
def validate(train,alpha,b,valid,tau):
    ...
    print " -> success rate = " , success_rate(alpha,b,train,valid,gaussian_kernel_function,tau)
    print "    Esvm =",E_svm
    return E_svm
...
def success_rate(alpha,b,train,valid,kernel_function,tau):
    print discriminant(valid[0][0],train[0],\
                             alpha,train[1],b,kernel_function,tau)
    return sum([1 for i in xrange(valid[1].size) \
        if discriminant(valid[0][i],train[0],\
                             alpha,train[1],b,kernel_function,tau)*\
            valid[1][i] > 0])/double(valid[1].size)
